...
def rvPlotResults(Dir, fname, dosave):
    ...
    # Dir = 'C:\Users\praja\OneDrive\UT MDPhD\White noise\Human tissue\Jan 25, 2018\Cell 3\Gain 20'
    ...
    for f in files:
        ...
        # plot traces of all trials - add titles, and make trace lines thinner, also ask Dr. Valiante which current input to graph
        ...
        # plot PCA
        pca = PCA(n_components=10).fit(spBins[f])
        result = pd.DataFrame(pca.transform(spBins[f]), columns=['PCA%i' % i for i in range(10)])
        # Plot initialisation
        fig = plt.figure()
        ax = Axes3D(fig)
